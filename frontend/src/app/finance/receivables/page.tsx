...
export default function FinanceReceivablesPage() {
  ...
  return (
    <main className="mx-auto max-w-[1320px] px-4 py-5 sm:px-8 sm:py-8 lg:px-12">
      ...
      {paymentSale ? <div role="dialog" aria-modal="true" aria-labelledby="payment-title" className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-[#151f36]/45 p-2 sm:p-4"><form onSubmit={submitPayment} className="max-h-[calc(100dvh-1rem)] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-4 shadow-2xl sm:p-6"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="finance-eyebrow">Collection receipt</p><h2 id="payment-title" className="mt-2 break-words text-xl font-extrabold sm:text-2xl">Record payment for {paymentSale.sale_id}</h2><p className="mt-2 text-sm text-[var(--navy-muted)]">Outstanding: {formatCurrency(paymentSale.balance)}</p></div><button type="button" onClick={() => setPaymentSale(null)} aria-label="Close payment form" className="grid h-11 w-11 shrink-0 place-items-center rounded-full text-2xl">×</button></div><div className="mt-5 grid gap-4 sm:mt-6 sm:grid-cols-2">
      ...
      </div><div className="mt-6 grid grid-cols-2 gap-3 sm:flex sm:justify-end"><button type="button" onClick={() => setPaymentSale(null)} className="rounded-lg border px-4 py-3 font-bold sm:px-5">Cancel</button><button disabled={submitting} className="finance-button px-3 sm:px-4">{submitting ? "Recording…" : "Record payment"}</button></div></form></div> : null}
      <PaymentDetailsDialog
