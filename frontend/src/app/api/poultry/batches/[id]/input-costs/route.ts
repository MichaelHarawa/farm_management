...
import type { InputCost } from "@/features/poultry/types";
...
export async function POST(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    const inputCost = await routeAuthenticatedBackendFetch<InputCost>(
      request,
    ...
    return NextResponse.json(inputCost, { status: 201 });
  } catch (error) {
