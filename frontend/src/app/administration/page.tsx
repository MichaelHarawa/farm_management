...
export default function AdministrationPage() {
  ...
  return <main className="min-h-screen bg-[var(--page-cream)] px-5 py-10 sm:px-8">
    <div className="mx-auto max-w-7xl">
      ...
      </PaginatedTableBody></table></section>
    </div>

    {showCreate ? <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-[#151f36]/45 p-2 sm:p-4" role="dialog" aria-modal="true"><form onSubmit={createUser} className="max-h-[calc(100dvh-1rem)] w-full max-w-3xl overflow-y-auto rounded-2xl bg-white p-4 sm:p-6"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><p className="finance-eyebrow">Technical access</p><h2 className="mt-2 text-xl font-extrabold sm:text-2xl">Create system user</h2></div><button type="button" onClick={() => setShowCreate(false)} aria-label="Close" className="grid h-11 w-11 shrink-0 place-items-center rounded-full text-2xl">×</button></div><div className="mt-5 grid gap-4 sm:mt-6 sm:grid-cols-2">
    ...
    </div><fieldset className="mt-5"><legend className="text-sm font-bold">System roles</legend><div className="mt-2 grid gap-2 sm:grid-cols-2">{ROLE_OPTIONS.map((role) => <label key={role.slug} className="flex items-center gap-2 rounded border p-3"><input type="checkbox" checked={form.role_slugs.includes(role.slug)} onChange={(event) => setForm({ ...form, role_slugs: event.target.checked ? [...form.role_slugs, role.slug] : form.role_slugs.filter((slug) => slug !== role.slug) })}/>{role.name}</label>)}</div></fieldset><div className="mt-6 grid grid-cols-2 gap-3 sm:flex sm:justify-end"><button type="button" onClick={() => setShowCreate(false)} className="rounded border px-4 py-3 font-bold sm:px-5">Cancel</button><button disabled={busy} className="finance-button px-3 text-[var(--navy)] sm:px-4">{busy ? "Creating…" : "Create user"}</button></div></form></div> : null}

    {editingUser ? <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-[#151f36]/45 p-2 sm:p-4" role="dialog" aria-modal="true" aria-labelledby="role-editor-title"><form onSubmit={saveRoles} className="max-h-[calc(100dvh-1rem)] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-4 shadow-2xl sm:p-6"><div className="flex items-start justify-between gap-3 sm:gap-4"><div className="min-w-0"><p className="text-label text-[var(--navy-muted)]">Access control</p><h2 id="role-editor-title" className="mt-2 break-words text-xl font-extrabold sm:text-2xl">Edit roles · {editingUser.username}</h2><p className="mt-2 text-sm text-[var(--navy-muted)]">Finance access is available to administrators, directors, farm managers, farm supervisors, and read-only stakeholders.</p></div><button type="button" onClick={() => setEditingUser(null)} aria-label="Close" className="grid h-11 w-11 shrink-0 place-items-center rounded-full text-2xl">×</button></div><fieldset className="mt-5 sm:mt-6"><legend className="sr-only">System roles</legend><div className="grid gap-3 sm:grid-cols-2">{ROLE_OPTIONS.map((role) => <label key={role.slug} className={`flex cursor-pointer items-center gap-3 rounded-xl border p-3 sm:p-4 ${roleDraft.includes(role.slug) ? "border-[var(--gold)] bg-[var(--gold-soft)]" : "border-[var(--line)]"}`}><input type="checkbox" checked={roleDraft.includes(role.slug)} onChange={(event) => setRoleDraft(event.target.checked ? [...roleDraft, role.slug] : roleDraft.filter((slug) => slug !== role.slug))}/><span><strong className="block">{role.name}</strong><span className="mt-1 block text-xs text-[var(--navy-muted)]">{roleAccessDescription(role.slug)}</span></span></label>)}</div></fieldset><div className="mt-6 grid grid-cols-2 gap-3 sm:flex sm:justify-end"><button type="button" onClick={() => setEditingUser(null)} className="rounded border px-4 py-3 font-bold sm:px-5">Cancel</button><button disabled={busy} className="finance-button px-3 sm:px-4">{busy ? "Saving…" : "Save roles"}</button></div></form></div> : null}

    {history ? <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-[#151f36]/45 p-2 sm:p-4" role="dialog" aria-modal="true"><section className="max-h-[calc(100dvh-1rem)] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-4 sm:p-6"><div className="flex items-start justify-between gap-3"><h2 className="min-w-0 break-words text-xl font-extrabold sm:text-2xl">Audit history · {history.user.username}</h2><button onClick={() => setHistory(null)} aria-label="Close" className="grid h-11 w-11 shrink-0 place-items-center rounded-full text-2xl">×</button></div><div className="mt-5 grid gap-3">{history.events.map((event) => <article key={event.id} className="rounded border p-4"><strong>{event.action.replaceAll("_", " ")}</strong><p className="text-sm text-[var(--navy-muted)]">{new Date(event.created_at).toLocaleString()} by {event.performed_by}</p></article>)}{!history.events.length ? <p>No account changes recorded.</p> : null}</div></section></div> : null}
  </main>;
}
