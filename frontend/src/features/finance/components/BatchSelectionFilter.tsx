...
export function BatchSelectionFilter({
  ...
  return (
    <div className="grid gap-4">
      ...
      {selectedIds.length ? (
        <div className="flex max-h-24 flex-wrap gap-2 overflow-y-auto" aria-label="Selected batches">
          {selectedIds.map((batchId) => (
            <button
              key={batchId}
              type="button"
              onClick={() => toggleBatch(batchId)}
              className="rounded-full border border-[var(--line)] bg-[var(--gold-soft)] px-3 py-1.5 text-xs font-extrabold text-[var(--navy)]"
      ...
      <div className="max-h-72 overflow-y-auto rounded-lg border border-[var(--line)] bg-white/55 p-2">
        {visibleBatches.length ? (
          <ul className="grid gap-1 sm:grid-cols-2 lg:grid-cols-3">
            {visibleBatches.map((batch) => (
              <li key={batch.id}>
                <label className="flex cursor-pointer items-start gap-3 rounded-md px-3 py-3 transition hover:bg-[var(--gold-soft)]">
                  <input
      ...
      {!selectedIds.length ? (
        <p className="text-sm font-semibold text-[var(--danger)]">
          Select one batch for a focused result or several batches for a combined result.
        </p>
      ) : null}
      {selectedIds.length >= MAX_BATCH_SELECTION ? (
        <p className="text-sm font-semibold text-[var(--navy-muted)]">
          The comparison limit is {MAX_BATCH_SELECTION} batches at a time.
          {selectedIds.length > MAX_BATCH_SELECTION
            ? " Remove batches before applying this URL selection."
            : ""}
        </p>
      ) : null}
    </div>
